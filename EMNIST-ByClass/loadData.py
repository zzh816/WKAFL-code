...
def TensorClip(Tensor, ClipBound):
    ...
    for i in range(Layers_num):
        Tensor[i] = Tensor[i]*torch.min(torch.ones(1), ClipBound/norm)
...
datasets = DatasetsLoader.loadDatesets(trainDataSize = 1000000, testDataSize = 120000, dataType=dataType)
#训练集，测试集
...
for itr in range(1, args.total_iterations + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        Loss_train += loss
        if itr > 4000:
            Gradients_Sample, _ = TensorClip(Gradients_Sample, args.Clip_Bound)

        # 聚合所有mini-batch上的噪声化梯度（聚合当前回合被抽取用户的噪声化梯度）
        for i in range(Layers_num):
            Collect_Gradients[i] += Gradients_Sample[i]
    # 利用平均化梯度更新服务器模型
    for i in range(Layers_num):
        Collect_Gradients[i] = Collect_Gradients[i]/args.K
...
with open('./实验结果/Asyn.txt', 'a+') as fl:
    fl.write('\n Results (UN is {}, K is {}, BZ is {}, LR is {})\n'.
             format(args.user_num, args.K, args.batch_size, args.lr))
    fl.write('Asyn = ' + str(logs['test_acc']))
    fl.write(';\n%' + str(test_acc))
