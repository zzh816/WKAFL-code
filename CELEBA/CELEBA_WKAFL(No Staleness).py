...
#################################聚合####################################
def aggregation(Collect_Gradients, K_Gradients, Layers_shape, args, Clip=False):
...
#测试集
test_loader = torch.utils.data.DataLoader(Datasets.celeba(data_root_test, IMAGES_DIR, 1835),
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for i in range(args.K):
        ...
        for j in range(Layers_num):
            Collect_Gradients[j] += Gradients_Sample[j] * 1./args.K

    if itr < 1000:
        Collect_Gradients = aggregation(Collect_Gradients, K_Gradients, Layers_shape, args)
    elif itr > 100:
        Collect_Gradients = aggregation(Collect_Gradients, K_Gradients, Layers_shape, args, Clip=True)

    #平均训练损失
    Loss_train /= (idx_outer + 1)
    #利用平均化梯度更新服务器模型并且把更新后的模型发送给对应学习者
    for idx_para, params_sever in enumerate(model.parameters()):
        params_sever.data.add_(-args.lr, Collect_Gradients[idx_para])
...
with open('./results/CELEBA_WKAFL_NoStaleness_testloss.txt', 'a+') as fl:
...
with open('./results/CELEBA_WKAFL_NoStaleness_testacc.txt', 'a+') as fl:
    fl.write('\n' + date + ' Results (UN is {}, K is {}, BZ is {}, LR is {}, CB1 is {}, '.
             format(args.users_total, args.K, args.batch_size, args.lr, args.CB1))
    fl.write('CB2 is {}, threshold is {}, total itr is {}, itr_test is {})\n'.
        format(args.CB2, args.threshold, args.itr_numbers, args.log_test))
    fl.write('WKAFL(No Staleness): ' + str(logs['test_acc']) + '\t')

with open('./results/CELEBA_WKAFL_NoStaleness_trainloss.txt', 'a+') as fl:
    fl.write('\n' + date + ' Results (UN is {}, K is {}, BZ is {}, LR is {}, CB1 is {}, '.
             format(args.users_total, args.K, args.batch_size, args.lr, args.CB1))
    fl.write('CB2 is {}, threshold is {}, total itr is {}, itr_test is {})\n'.
             format(args.CB2, args.threshold, args.itr_numbers, args.log_test))
