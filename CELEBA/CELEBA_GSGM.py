...
for itr in range(1, args.itr_numbers + 1):
    federated_train_loader = sy.FederatedDataLoader(Federate_Dataset, worker_have_list=worker_have_list,
    ...
    for i in range(Layers_num):
        Gradients_unbiased[i] = args.momentum * Gradients_Momentum[i] + Collect_Gradients[i]
        Gradients_pa[i] = Gradients_pa[i] + Gradients_unbiased[i] / args.users_total
